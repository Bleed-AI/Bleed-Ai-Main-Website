...
// Clone of RepliesMarquee with a single fix: the label row alignment.
// Original `.email-label-row` class centers the label, which doesn't match
// the left-aligned heading below it. We override justifyContent inline so
// the label sits flush-left with the H2.
...
export default function RepliesMarqueeTest() {
  return (
          ...
          <Reveal className="email-marquee">
            <div className="email-track">
              {[...replies, ...replies].map((r, i) => {
                const body =
                  !r.redact || !r.redact.length
                    ? r.body
                    : (() => {
                        const pattern = new RegExp(`(${r.redact.map((t) => t.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|")})`, "g");
                        return r.body.split(pattern).map((part, j) =>
                          r.redact!.includes(part) ? (
                            <span key={j} className="email-redact">{part}</span>
                          ) : (
                            part
                          )
                        );
