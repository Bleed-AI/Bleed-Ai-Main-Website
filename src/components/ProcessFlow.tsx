...
// Everything is inlined in this one component so scoped <style jsx> applies.
...
type Slide = { title: string; kind: string };
const columns: { step: string; caption: string; slides: Slide[] }[] = [
  {
    step: "01",
    caption: "We Find The Right Buyers",
    slides: [
      { title: "// selecting best-fit accounts", kind: "accounts" },
      { title: "// sharpening your value prop", kind: "valueprop" },
    ],
  },
  {
    step: "02",
    ...
    slides: [
      { title: "// email · linkedin · calls, in sync", kind: "sequence" },
      { title: "// automated lead enrichment", kind: "enrich" },
    ],
  },
  {
    step: "03",
    ...
    slides: [
      { title: "// a/b testing every message", kind: "bars" },
      { title: "// weekly performance analysis", kind: "lines" },
    ],
  },
];
  ...
  return (
    <section id="process">
                ...
                {ci > 0 && (
                  <div className="pf-connector" aria-hidden="true">
                    <svg viewBox="0 0 80 220" preserveAspectRatio="none">
                      <g fill="none" stroke="rgba(255,90,80,0.38)" strokeWidth="1.4" strokeDasharray="4 5" className="pf-dash">
                      ...
                      <circle r="2.6" fill="#ff6b67" className="pf-spark">
                        ...
                        <div key={i} className={`pf-slide${i === active ? " on" : ""}`}>
                          ...
                          {s.kind === "accounts" && (
                            <div className="pf-accounts">
                              {accounts.map((a, ai) => (
                                <div key={a} className={`pf-acct${ai === 1 ? " sel" : ""}`}>
                          ...
                          {s.kind === "sequence" && (
                            <div className="pf-seq">
                              ...
                              {steps.map((st, si) => (
                                <div key={st.label} className="pf-seq-row" style={{ animationDelay: `${si * 0.9}s`, ["--c" as string]: st.color } as React.CSSProperties}>
                                  <span className="pf-seq-day">{st.day}</span>
                                  <span className="pf-seq-ico">{st.icon}</span>
                                  <span className="pf-seq-label">{st.label}</span>
                                </div>
                              ))}
                            </div>
                          )}
                          ...
                          {s.kind === "enrich" && (
                            <div className="pf-enrich">
                              {enrich.map((e, ei) => (
                                <div key={e.label} className="pf-enrich-row" style={{ animationDelay: `${ei * 0.5}s` }}>
                          ...
                          {s.kind === "bars" && (
                            <div className="pf-chart">
                              <div className="pf-bars">
                                {bars.map((b, bi) => (
                                  <span key={bi} className={`pf-bar ${b.kind}`} style={{ height: `${b.h}%`, animationDelay: `${bi * 0.18}s` }} />
                                ))}
                              </div>
      ...
      <style jsx>{`
        ...
        .pf-vp-tag { font-family: "JetBrains Mono", ui-monospace, monospace; font-size: 11px; color: #6a7192; padding-top: 2px; }
        .pf-vp-row.v1 { color: #6a7192; }
        .pf-vp-row.v2 {
          color: #f0f2fa; font-weight: 600;
          border-color: rgba(255, 61, 56, 0.45); background: rgba(255, 61, 56, 0.08);
        ...
        .pf-seq-rail { position: absolute; left: 0; top: 8px; bottom: 8px; width: 2px; background: rgba(255, 255, 255, 0.08); border-radius: 2px; overflow: hidden; }
        .pf-seq-fill { position: absolute; inset: 0; background: linear-gradient(#ff3d38, #b1130f); transform: scaleY(0); transform-origin: top; animation: seqFill 3s ease-in-out infinite; }
        ...
        .pf-seq-row {
          ...
          border: 1px solid rgba(255, 255, 255, 0.08); background: #0d0d15; opacity: 0; animation: seqRow 3s ease-in-out infinite;
        }
        ...
        .pf-enrich-row {
          ...
          border: 1px solid rgba(255,255,255,0.08); background: #0d0d15; opacity: 0; animation: seqRow 3.4s ease-in-out infinite;
        }
        .pf-enrich-label { font-family: "JetBrains Mono", ui-monospace, monospace; font-size: 12px; color: #6a7192; min-width: 48px; }
        .pf-enrich-value { font-family: "JetBrains Mono", ui-monospace, monospace; font-size: 14px; font-weight: 600; color: #e4e7f2; flex: 1; }
        .pf-enrich-badge { width: 26px; height: 26px; border-radius: 7px; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 800; color: #fff; }

        .pf-chart { position: relative; }
        .pf-bars { display: flex; align-items: flex-end; justify-content: center; gap: 16px; height: 150px; }
        .pf-bar { width: 34px; border-radius: 5px 5px 0 0; transform: scaleY(0); transform-origin: bottom; animation: barGrow 3.4s cubic-bezier(0.22, 1, 0.36, 1) infinite; }
        ...
        .pf-legend { display: flex; justify-content: center; gap: 20px; margin-top: 16px; font-family: "JetBrains Mono", ui-monospace, monospace; font-size: 12.5px; color: #9099b8; }
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .pf-vp-row.v2, .pf-line-path { animation: none !important; }
